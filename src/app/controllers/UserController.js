...
class UserController {
  async store(req, res) {
    const schema = Yup.object().shape({
      ...
      telephone: Yup.string().required().max(20),
      date_birth: Yup.date().required(),
      gender: Yup.string().required().max(1),
    ...
    if (UserExists) {
      return res.status(400).json({ error: "User or CPF already exists!" });
    }
  ...
  async update(req, res) {
    ...
    if (cpf && cpf !== user.cpf) {
      ...
      if (cpfExists) {
        return res.status(400).json({ erro: "CPF already exists!" });
      }
    }

    if (oldPassword && !(await user.checkPassword(oldPassword))) {
      return res.status(400).json({ erro: "Password does not math!" });
    }
    ...
    return res.json({ id_user, name, email, telephone, gender, date_birth });
  }
  ...
  async delete(req, res) {
    ...
    if (!user) {
      return res.status(400).json({ error: "Users not exists!" });
    }

    await user.destroy(req.params.id_user);

    return res.json({ message: "Users excluded with success!" });
  }
}
